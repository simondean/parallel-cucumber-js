var Main = function(options) {
  ...
  self.run = function(callback) {
    configFilePath = self._options.configFilePath;
  ...
  self._createFormatters = function(options, callback) {
    ...
    self._options.formats.forEach(function(format) {
      ...
      var formatter = parts[0];
      var outFilePath = parts.slice(1).join(':');
      var formatterType;

      if (formatter.toLowerCase() == 'json') {
        formatterType = ParallelCucumber.Formatters.JsonFormatter;
      }
      else if (formatter.toLowerCase() == 'progress') {
        formatterType = ParallelCucumber.Formatters.ProgressFormatter;
      }
      else {
        try {
          formatterType = require(Path.resolve(formatter));
        }
        catch (err) {
          callback({ message: 'Failed to load custom formatter', formatter: formatter, innerError: err });
          return;
        }
      }
  ...
  self._findFeatures = function(options, callback) {
